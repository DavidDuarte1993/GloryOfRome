...
void AAICharacter::CreateAndAttachWeapons()
{
	...
	if (Shield != nullptr) {
		...
		APickableItemBase* NewShield =
			Cast<APickableItemBase>(GetWorld()->SpawnActor(Shield, &Position, &Rotation));
	...
	if (Weapon != nullptr) {
		...
		APickableWeapon* NewWeapon =
			Cast<APickableWeapon>(GetWorld()->SpawnActor(Weapon, &Position, &Rotation));
		...
		if (WeaponCollider != nullptr) {
			...
			WeaponCollider->OnComponentBeginOverlap.AddDynamic(this, 
				&AAICharacter::OnWeaponBeginOverlap);
